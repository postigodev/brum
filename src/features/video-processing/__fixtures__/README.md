...
## Phone input containers (#29)

These commands remux synthetic media into actual QuickTime containers (not just renamed MP4s).
FFmpeg is only a developer fixture generator; it is not part of the application.

```powershell
ffmpeg -i src/features/video-processing/__fixtures__/h264-directional.mp4 -c copy -f mov -movflags +faststart -y src/features/video-processing/__fixtures__/h264-directional.mov
ffmpeg -i src/features/video-processing/__fixtures__/hevc-video.mp4 -c copy -tag:v hvc1 -f mov -movflags +faststart -y src/features/video-processing/__fixtures__/hevc-video.mov
ffmpeg -f lavfi -i "testsrc2=size=160x120:rate=10:duration=1" -c:v libvpx-vp9 -y src/features/video-processing/__fixtures__/unsupported-video.mp4
```

MOV AVC preserves the four directional states and six-second duration. HEVC MP4/MOV contain
ten 160x120 frames over one second. HEVC decoding/encoding browser coverage skips only when
the actual track reports no decode capability; the forced-unavailable path always runs.
